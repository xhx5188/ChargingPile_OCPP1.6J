# --*-- encoding:utf-8 --*--
"""
@File:test_Recurrent_cpVoltN.py
@Time:2022/1/21 11:51
...
async def test_Recurrent_cpVoltN(event_loop):
    ...
    for i in range(1000):
        ...
        # 重启充电桩
        logging.info("%s", time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
        clearTriggerMessage()
        response = await service.reset(event_loop, "Soft")
        assert response[0].status == RegistrationStatus.accepted
        logging.info("********正在重启********请稍后********")
