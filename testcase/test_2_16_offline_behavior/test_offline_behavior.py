...
async def test_offline_start_transaction_1(event_loop):
    ...
    # 等待本地发送充电请求
    flag, _ = await waitRequest("start_transaction", 40)
    assert flag == True
...
async def test_offline_start_transaction_2(event_loop):
    ...
    # 刷卡
    logging.info("[请刷一张未绑定卡(非法卡)启动充电:]")
    ...
    # 等待本地发送充电请求
    flag, msg = await waitRequest("start_transaction", 40)
    assert flag == True
    ...
    # 判断枪的状态
    status = await waitConnectorStatus(1, "SuspendedEVSE", 40)
    ...
    # 判断枪的停止充电后的状态
    status = await waitConnectorStatus(1, "Preparing", 40)
    assert status == "Preparing"
...
async def test_stop_transaction(event_loop):
    ...
    result = await service.getConfiguration(event_loop, ["AuthorizeRemoteTxRequests"])
    assert result[0]['value'] == "true"
    assert result[0]["key"] == "AuthorizeRemoteTxRequests"
